...
import { Phone, Gauge, ChevronRight, Download, CheckCircle2 } from "lucide-react";
...
export default function LoginPage() {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // If PWA is installable, show the button
      if (deferredPrompt) {
        setShowInstallBtn(true);
      } else {
        // If already installed or not supported, redirect
        setTimeout(() => router.push("/mode-selection"), 1000);
      }
    } catch (err: any) {
      ...
      <div className="w-full max-w-sm space-y-12">
        ...
        {!showInstallBtn ? (
          <form onSubmit={handleLogin} className="space-y-6">
            ...
            <button
              type="submit"
              disabled={loading || phone.length < 10 || authSuccess}
          ...
          <div className="space-y-6 animate-in zoom-in-95 duration-500">
            <div className="glass p-8 rounded-3xl border-primary/20 text-center space-y-4">
              <div className="w-16 h-16 bg-primary/10 rounded-2xl flex items-center justify-center mx-auto mb-2">
                <Download className="text-primary animate-bounce" size={32} />
              </div>
              <h3 className="text-xl font-black italic text-white uppercase tracking-tighter">Instalar Velocímetro</h3>
              <p className="text-[10px] text-muted-foreground uppercase leading-relaxed">
                Baixe o aplicativo para ter acesso total à telemetria em tempo real e modo offline.
              </p>
            </div>
            
            <button
              onClick={handleInstallApp}
              className="w-full py-6 rounded-2xl bg-white text-black font-black text-sm tracking-[0.2em] uppercase shadow-[0_0_50px_rgba(255,255,255,0.2)] hover:scale-[1.02] active:scale-95 transition-all flex items-center justify-center gap-3"
            >
              BAIXAR VELOCÍMETRO
              <Download size={20} />
            </button>
            ...
            <button 
              ...
              className="w-full text-[10px] font-bold text-muted-foreground uppercase tracking-widest hover:text-white transition-colors"
            >
